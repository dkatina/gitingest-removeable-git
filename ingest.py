...
def ingest(source: str, max_file_size: int = 10 * 1024 * 1024, include_patterns: Union[List[str], str] = None, exclude_patterns: Union[List[str], str] = None, output: str = None, remove_git=False) -> str:
    try:
        query = parse_query(source, max_file_size, False, include_patterns, exclude_patterns)     
        if query['url']:
            asyncio.run(clone_repo(query=query, remove_git=remove_git))
